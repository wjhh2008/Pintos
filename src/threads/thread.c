...
thread_block (void) 
{
  ...
  //printf("[%s] is now blocked! ",thread_current()->name);
  thread_current ()->status = THREAD_BLOCKED;
  schedule ();
  //printf("[%s] is now running! ",thread_current()->name);
}
...
thread_unblock (struct thread *t) 
{
  ...
  //printf("[%s] is now unblocked  [%s] is current thread pri: [%d %d]\n",t->name,thread_current()->name, t->priority,thread_current()->priority);
  //if (thread_current()->priority <= t->priority)
//	thread_yield();
...
int
thread_less(struct list_elem *elem1, struct list_elem *elem2, void *aux)
{
	struct thread *t1, *t2;
